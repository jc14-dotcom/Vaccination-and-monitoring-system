...
<div class="flex flex-col pb-8">
    ...
    <section class="relative overflow-hidden rounded-2xl mb-6 ring-1 ring-primary-300/40 bg-gradient-to-r from-primary-600 to-primary-800">
        <div class="relative px-5 py-6 text-white flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-3" data-animate>
                <span class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/25">
                    <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12h18M3 6h18M3 18h18"/></svg>
                </span>
                <div>
                    <h1 class="text-xl md:text-2xl font-semibold leading-tight">Dashboard Overview</h1>
                    <p class="text-xs text-white/85">Quick insights and upcoming schedules</p>
                </div>
            </div>
    ...
    <!-- KPI Cards -->
    <section class="grid gap-5 sm:grid-cols-2 xl:grid-cols-3 mb-6">
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-5 flex items-center gap-4" data-animate>
        ...
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-5 flex items-center gap-4" data-animate>
            ...
            <div>
                ...
                <p class="text-2xl font-bold text-gray-900">{{ $vaccinated ?? 0 }}</p>
            </div>
        </div>
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-gray-200 p-5 flex items-center gap-4" data-animate>
            <span class="w-12 h-12 rounded-xl bg-amber-50 text-amber-700 inline-flex items-center justify-center ring-1 ring-amber-100">
                <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M12 20h9M12 4h9M4 9h16M4 15h16"/></svg>
            </span>
            <div>
                <p class="text-sm text-gray-500">Feedback Complete</p>
                <p class="text-2xl font-bold text-gray-900">{{ $feedbackCount ?? 0 }}</p>
            </div>
        </div>
    </section>

    <!-- Charts -->
    <section class="grid gap-5 md:grid-cols-2 xl:grid-cols-3 mb-6">
        <div class="rounded-2xl bg-white shadow ring-1 ring-gray-200 p-5 h-80" data-animate>
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-sm font-semibold text-gray-700">Monthly Statistics</h2>
            </div>
            <canvas id="monthlyChart" class="h-full w-full"></canvas>
        </div>
        <div class="rounded-2xl bg-white shadow ring-1 ring-gray-200 p-5 h-80" data-animate>
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-sm font-semibold text-gray-700">Vaccination Status</h2>
            </div>
            <canvas id="vaccineStatusChart" class="h-full w-full"></canvas>
        </div>
        <div class="rounded-2xl bg-white shadow ring-1 ring-gray-200 p-5 h-80 md:col-span-2 xl:col-span-1" data-animate>
            <div class="flex items-center justify-between mb-3">
                <h2 class="text-sm font-semibold text-gray-700">Vaccination Distribution</h2>
            </div>
    ...
    <section class="rounded-2xl bg-white shadow ring-1 ring-gray-200 overflow-hidden mb-6" data-animate>
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-gray-700">Upcoming Vaccinations</h2>
            <a href="{{ route('health_worker.vaccination_status') }}" class="inline-flex items-center gap-2 text-sm font-semibold text-primary-700 hover:text-primary-800">
                View all
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
            </a>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full text-sm">
                <thead class="bg-gradient-to-r from-[#7a5bbd] to-[#5a3f99] text-white">
                    <tr>
                        <th class="px-4 py-3 text-left font-semibold">Patient Name</th>
                        <th class="px-4 py-3 text-left font-semibold">Vaccine</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @if(isset($upcomingVaccinations) && count($upcomingVaccinations) > 0)
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 whitespace-nowrap">{{ $vaccination->patient_name ?? 'N/A' }}</td>
                            <td class="px-4 py-3">{{ $vaccination->vaccine_name ?? 'N/A' }}</td>
                            <td class="px-4 py-3 whitespace-nowrap">{{ $vaccination->scheduled_date ?? 'N/A' }}</td>
                            <td class="px-4 py-2">
                                <a href="{{ url('/vaccination_form/' . ($vaccination->patient_id ?? 0)) }}" class="inline-flex items-center gap-1 rounded-lg bg-[#7a5bbd] hover:bg-[#6d51ad] text-white text-xs font-semibold px-3 py-1.5 shadow-sm ring-1 ring-[#7a5bbd]/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#7a5bbd]/50">
                    ...
                    @else
                        <tr>
                            <td colspan="4" class="px-4 py-4 text-center text-gray-500">No upcoming vaccinations</td>
                        </tr>
                    @endif
                </tbody>
            </table>
    ...
    <!-- Feedback Analysis -->
    <section class="rounded-2xl bg-white shadow ring-1 ring-gray-200" data-animate>
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 px-5 py-4 border-b border-gray-100">
            <h2 class="text-sm font-semibold text-gray-700">Feedback Analysis</h2>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            new Chart(vaccineDistCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Vaccinations',
                        data: {!! json_encode($vaccineDistData ?? [65,59,80,81,56,55,40]) !!},
