...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Restore expanded years for both tabs
    const expandedYears = JSON.parse(localStorage.getItem('expandedYears') || '[]');
    
    expandedYears.forEach(year => {
        // Check both regular and deleted year sections
        const content = document.getElementById(`year-${year}`);
        const arrow = document.getElementById(`arrow-${year}`);
        const deletedContent = document.getElementById(`year-deleted-${year}`);
        const deletedArrow = document.getElementById(`arrow-deleted-${year}`);
        ...
        if (deletedContent && deletedArrow) {
            deletedContent.style.maxHeight = deletedContent.scrollHeight + 'px';
            deletedArrow.classList.remove('-rotate-90');
        }
    });
});
